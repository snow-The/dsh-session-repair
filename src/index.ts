...
export function apply(ctx: Ctx) {
  ctx.tools.register({
    ...
    description: 
      ...
      'Returns a compact summary by default; pass verbose:true for the per-file detail.',
    parameters: {
      type: 'object',
      properties: {
    ...
    execute: async (args: Json) => {
      ...
      // Compact by default: the full report is ~5k tokens per inspected file — a single call
      // to this tool has measured 736k tokens, which can swallow a whole context window. The
      // agent only needs the counts plus enough evidence to decide what to do next.
      if ((args as { verbose?: boolean }).verbose !== true) {
